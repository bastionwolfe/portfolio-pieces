  ...
  //AoS integrations
    ...
    void AoSIntegrateVerlet(PhysicsObjAoS& AoSObj, float dt) {
        AoS_Vec3 temp = AoSObj.position;  // Fix here: AoSObj.position, not just position
        AoSObj.position = AoSObj.position * 2.0f - AoSObj.previousPosition + AoSObj.acceleration * (dt * dt);
        AoSObj.previousPosition = temp;
    }
    
    //SoA integrations
    
    void SoAIntegrateVelocities(PhysicsObjSoA& SoAObj, float dt) {
    ...
    void SoAIntegrateVerlets(PhysicsObjSoA& SoAObj, float dt) {
         for (size_t i = 0; i < SoAObj.positions.size(); ++i) {
            ...
            SoAObj.previousPositions[i] = temp;
        }
    }
    ...
    void AoSoA8IntegrateVelocities(PhysicsObjAoSoA8& ObjAoSoA8, float dt) {
    ...
    void AoSoA8IntegrateVerlets(PhysicsObjAoSoA8& ObjAoSoA8, float dt) {
    int objectCount = ObjAoSoA8.positions.getSize();
    ...
    for (int i = 0; i < objectCount; ++i) {
        ...
        ObjAoSoA8.previousPositions.setX(i, px);
        ObjAoSoA8.previousPositions.setY(i, py);
        ObjAoSoA8.previousPositions.setZ(i, pz);
